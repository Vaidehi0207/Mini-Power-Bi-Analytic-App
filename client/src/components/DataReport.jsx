...
const DataReport = ({ file, onClose, onExplorePBI }) => {
    ...
    const handleDownload = async () => {
        try {
            ...
            const response = await fetch(`http://localhost:5000/api/data/download/${file._id}`, {
                headers: {
                ...
                {activeTab === 'workflow' ? (
                    ...
                    <>
                        ...
                        <div className="report-details">
                            ...
                            {/* Alteryx Premium Insights */}
                            {file.processingType === 'premium' && (
                                <div className="premium-insights">
                                    {audit.blendingInsights && (
                                        <div className="insight-section">
                                            <h4><span className="premium-label">Data Blending</span> (Cross-Source)</h4>
                                            <div className="blending-box">
                                                <div className="source-list">
                                                    {audit.blendingInsights.sources_joined.map(s => (
                                                        <div key={s} className="source-item"><Database size={14} /> {s}</div>
                                                    ))}
                                                </div>
                                                <div className="blend-stats">
                                                    <div className="blend-stat">
                                                        <GitMerge size={16} />
                                                        <span>{audit.blendingInsights.match_type}: <strong>{audit.blendingInsights.records_matched}</strong> records</span>
                                                    </div>
                                                    {audit.blendingInsights.join_keys && (
                                                        <div className="blend-stat-detail">
                                                            <span>Keys: <code>{audit.blendingInsights.join_keys.join(', ')}</code></span>
                                                            {audit.blendingInsights.fuzzy_confidence && <span> Confidence: <strong style={{ color: '#4ade80' }}>{audit.blendingInsights.fuzzy_confidence}</strong></span>}
                                                        </div>
                                                    )}
                                                </div>
                                            </div>
                                    ...
                                    {audit.schemaValidation && (
                                        <div className="insight-section">
                                            <h4><span className="premium-label">Schema Intelligence</span></h4>
                                            <div className="schema-box">
                                                <div className="schema-status">
                                                    <ShieldCheck size={18} />
                                                    <span>{audit.schemaValidation.status}</span>
                                                </div>
                                                <div className="mapping-list">
                                                    {Object.entries(audit.schemaValidation.auto_mapped_fields).map(([from, to]) => (
                                    ...
                                    {audit.fuzzyMatchMatches && (
                                        <div className="insight-section">
                                            <h4><span className="premium-label">Fuzzy Match Result</span> (AI-Powered)</h4>
                                            <div className="fuzzy-list">
                                                {audit.fuzzyMatchMatches.map((m, i) => (
                                                    <div key={i} className="fuzzy-item">
                                                        <span className="fuzzy-orig">{m.original}</span>
                                                        <span className="fuzzy-arrow">→</span>
                                                        <span className="fuzzy-match">{m.match}</span>
                                                        <span className="fuzzy-score">{m.score}% Match</span>
                                                    </div>
                                                ))}
                                            </div>
                                        </div>
                                    ...
                                    {audit.spatialAnalysis && (
                                        <div className="insight-section">
                                            <h4><span className="premium-label">Spatial Discovery</span></h4>
                                            <div className="spatial-box">
                                                <div className="spatial-stat">
                                                    <span>Records Enriched</span>
                                                    <strong>{audit.spatialAnalysis.records_enriched}</strong>
                                                </div>
                                                <div className="spatial-stat">
                                                    <span>Avg Distance</span>
                                                    <strong>{audit.spatialAnalysis.avg_distance}</strong>
                                                </div>
                                                <div className="spatial-stat" style={{ color: '#fbbf24', fontSize: '12px' }}>
                                                    <Globe size={14} /> {audit.spatialAnalysis.trade_area_analysis}
                                                </div>
                                                <div className="spatial-tags">
                                                    {audit.spatialAnalysis.new_fields.map((f, i) => (
                                                        <span key={i} className="spatial-tag">{f}</span>
                                                    ))}
                                                </div>
                                            </div>
                                        </div>
